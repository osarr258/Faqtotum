...
async def register(data: RegisterInput, request: Request):
    ...
    token = await create_session(user_id, request=request)
...
async def login(data: LoginInput, request: Request):
    ...
    await security.check_login_rate_limit(db, data.email, ip)
    user = await db.users.find_one({"email": data.email.lower()})
    if not user or not user.get("password") or not verify_password(data.password, user["password"]):
        await security.record_login_attempt(db, data.email, ip, success=False)
        await security.audit_log(
    ...
    await security.record_login_attempt(db, data.email, ip, success=True, user_id=user["user_id"])
    await security.audit_log(
...
# ==================================================================
# Sprint 8 — Security, Privacy, Authentication & Platform Integrity
# ==================================================================
...
async def security_overview(user=Depends(get_current_user)):
    score = await security.compute_security_score(db, user["user_id"])
    active_sessions = await db.user_sessions.count_documents({
        "user_id": user["user_id"], "revoked": {"$ne": True},
    })
    recent_events = await security.list_audit_logs(db, actor_id=user["user_id"], limit=10)
    mfa = await security.mfa_status(db, user["user_id"])
    return {
...
async def get_sessions(user=Depends(get_current_user)):
    sessions = await security.list_user_sessions(db, user["user_id"])
...
async def revoke_session_endpoint(session_id: str, user=Depends(get_current_user)):
    ok = await security.revoke_session(db, user["user_id"], session_id)
    if not ok:
        raise HTTPException(status_code=404, detail="Session introuvable")
    await security.audit_log(
...
@api_router.post("/security/sessions/revoke-others")
async def revoke_other_sessions(user=Depends(get_current_user)):
    current_token = user.get("_current_token", "")
    count = await security.revoke_all_sessions_except(db, user["user_id"], current_token)
    await security.audit_log(
...
# ----- Password Change -----
@api_router.post("/security/password/change")
async def change_password(data: PasswordChangeInput, user=Depends(get_current_user), request: Request = None):
    ...
    await db.users.update_one(
        {"user_id": user["user_id"]},
        {"$set": {"password": hash_password(data.new_password), "password_changed_at": now_utc().isoformat()}},
    )
    await security.audit_log(
...
async def my_audit_logs(user=Depends(get_current_user), limit: int = 50):
    logs = await security.list_audit_logs(db, actor_id=user["user_id"], limit=min(200, max(1, limit)))
    for log in logs:
        log.pop("hash", None)  # Don't expose full hash to end user
...
async def audit_chain_verify(user=Depends(require_roles("admin", "super_admin"))):
    result = await security.verify_audit_chain(db, sample=500)
    return result


@api_router.get("/security/audit/all")
async def all_audit_logs(
    limit: int = 100, action: Optional[str] = None, severity: Optional[str] = None,
    user=Depends(require_roles("admin", "super_admin")),
):
    logs = await security.list_audit_logs(db, action=action, severity=severity, limit=min(500, limit))
    return {"logs": logs, "total": len(logs)}


# ----- GDPR / RGPD -----
@api_router.get("/security/gdpr/consents")
...
@api_router.post("/security/gdpr/consents")
...
@api_router.get("/security/gdpr/export")
async def gdpr_export(user=Depends(get_current_user)):
    data = await security.export_user_data(db, user["user_id"])
    await security.audit_log(
...
@api_router.delete("/security/gdpr/account")
async def gdpr_delete_account(user=Depends(get_current_user)):
    result = await security.anonymize_user(db, user["user_id"])
    await security.audit_log(
...
# ----- MFA -----
@api_router.get("/security/mfa")
async def get_mfa(user=Depends(get_current_user)):
    return await security.mfa_status(db, user["user_id"])


@api_router.post("/security/mfa/prepare")
async def prepare_mfa(data: MfaPrepareInput, user=Depends(get_current_user)):
    result = await security.mfa_prepare(db, user["user_id"], method=data.method)
    await security.audit_log(
...
async def verify_mfa(data: MfaVerifyInput, user=Depends(get_current_user)):
    result = await security.mfa_verify_stub(db, user["user_id"], data.code)
    await security.audit_log(
...
@api_router.delete("/security/mfa")
async def disable_mfa(user=Depends(get_current_user)):
    result = await security.mfa_disable(db, user["user_id"])
    await security.audit_log(
...
# ----- Biometrics (architecture stub) -----
@api_router.post("/security/biometrics/register")
async def register_biometric(data: BiometricRegisterInput, user=Depends(get_current_user)):
    result = await security.biometrics_register(db, user["user_id"], data.device_id, data.public_key)
    await security.audit_log(
...
@api_router.get("/security/biometrics")
...
async def get_roles(user=Depends(get_current_user)):
    """List roles available on the platform + current user role."""
    return {
        "current_role": user.get("role"),
        "roles": [
            {"key": k, "label": v["label"], "weight": v["weight"]}
            for k, v in security.ROLES.items()
        ],
    }


# ----- Admin: list all sessions (super_admin only) -----
@api_router.get("/security/admin/sessions")
...
# ----- User self-service: profile update, notifications, payment methods -----
...
async def update_me(data: UserUpdateInput, user=Depends(get_current_user)):
    ...
    await security.audit_log(
        ...
        target=user["user_id"], metadata={"fields": list(update.keys())}, severity="info",
    )
    new_user = await db.users.find_one({"user_id": user["user_id"]}, {"_id": 0, "password": 0})
...
@api_router.post("/users/me/notifications")
async def set_notif_prefs(data: NotifPrefsInput, user=Depends(get_current_user)):
    update = {k: v for k, v in data.dict(exclude_none=True).items()}
    update["user_id"] = user["user_id"]
    update["updated_at"] = now_utc().isoformat()
    await db.notif_prefs.update_one({"user_id": user["user_id"]}, {"$set": update}, upsert=True)
    return update


@api_router.get("/users/me/payment-methods")
async def list_payment_methods(user=Depends(get_current_user)):
    cursor = db.payment_methods.find({"user_id": user["user_id"]}, {"_id": 0}).sort("created_at", -1)
    methods = await cursor.to_list(20)
    return {"methods": methods}


class PaymentMethodInput(BaseModel):
    brand: str  # "visa" | "mastercard" | "apple_pay" | ...
    last4: str
...
@api_router.post("/users/me/payment-methods")
async def add_payment_method(data: PaymentMethodInput, user=Depends(get_current_user)):
    pm_id = new_id("pm")
    doc = {
        "pm_id": pm_id,
        "user_id": user["user_id"],
...
@api_router.delete("/users/me/payment-methods/{pm_id}")
async def delete_payment_method(pm_id: str, user=Depends(get_current_user)):
    result = await db.payment_methods.delete_one({"pm_id": pm_id, "user_id": user["user_id"]})
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Introuvable")
    return {"ok": True}


@api_router.post("/users/me/payment-methods/{pm_id}/default")
async def set_default_payment_method(pm_id: str, user=Depends(get_current_user)):
    pm = await db.payment_methods.find_one({"pm_id": pm_id, "user_id": user["user_id"]})
    if not pm:
        raise HTTPException(status_code=404, detail="Introuvable")
    await db.payment_methods.update_many({"user_id": user["user_id"]}, {"$set": {"is_default": False}})
